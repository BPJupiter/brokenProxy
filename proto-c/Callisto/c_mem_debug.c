...
STMemAllocLine c_alloc_lines[1024];
unsigned int c_alloc_linc_count = 0;
...
int c_debug_memory()
{
  ...
  for (i = 0; i < c_alloc_linc_count; i++)
  {
...
void *c_debug_mem_realloc(void *pointer, unsigned int size, char *file, unsigned int line)
{
  ...
  if (i == c_alloc_linc_count)
  {
    printf("EUROPA Mem debugger error. Trying to reallocate pointer %p in %s line %u. Pointer has never been allocated\n", pointer, file, line);
    for (i = 0; i < c_alloc_linc_count; i++)
    {
      for (j = 0; j < c_alloc_lines[i].alloc_count; j++)
      {
...
unsigned int c_debug_mem_consumption()
{
