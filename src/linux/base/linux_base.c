...
internal Dense_Time lnx_dense_time_from_timespec(timespec in)
{
    Dense_Time result = 0;
    {
        ...
        Date_Time date_time = lnx_date_time_from_tm(tm_time, in.tv_nsec / Million(1));
...
{
    LNX_Safe_Call_Chain *chain = lnx_safe_call_chain;
...
internal Date_Time now_time_universal(void)
{
    ...
    Date_Time result = lnx_date_time_from_tm(universal_tm, 0);
    return result;
}
...
internal Date_Time local_from_universal_time(Date_Time *dt)
{
    ...
    // local tm -> Date_Time
    Date_Time result = lnx_date_time_from_tm(local_tm, 0);
    return result;
}
...
{
    if (MemoryZeroStruct(&handle)) { return; }
...
{
    if (MemoryZeroStruct(&handle)) { return 0; }
    int id = (int)handle.u64[0];
...
///////////////////////////////////////////////////////
// @per_os_impl Synchronisation Primitive Functions
...
internal bool32 cond_var_wait(CondVar cv, Mutex mutex, u64 endt_us)
{
    ...
    bool32 result = (wait_result != ETIMEDOUT);
    return result;
}
...
internal bool32 file_set_times(File file, Date_Time date_time)
{
    if(file_match(file, file_zero())) { return 0; }
    int fd = (int)file.u64[0];
    ...
    bool32 good = (futimens_result != -1);
    return good;
}

internal File_Properties properties_from_file(File file)
{
    if(file_match(file, file_zero())) { return (File_Properties){0}; }
    int fd = (int)file.u64[0];
...
{
    if(file_match(file, file_zero())) { return (File_ID){0}; }
    int fd = (int)file.u64[0];
    struct stat fd_stat = {0};
    int fstat_result = fstat(fd, &fd_stat);
    File_ID id = {0};
...
internal bool32 copy_file_path(String8 dst, String8 src)
{
    ...
    if(!file_match(src_h, file_zero()) &&
       !file_match(dst_h, file_zero())) {
        ...
        File_Properties src_props = properties_from_file(src_h);
...
{
    File_Iter *base_iter = push_array(arena, File_Iter, 1);
...
{
    bool32 good = 0;
    LNX_File_Iter *lnx_iter = (LNX_File_Iter *)iter->memory;
    for(;lnx_iter->dir != 0;) {
        // rjf: get next entry
    ...
    if(file_actions_init_code == 0) {
        ...
        if(attr_init_code == 0) {
            ...
            char **argv = push_array(scratch.arena, char *, params->cmd_line.node_count + 1);
            {
                ...
                u64 arg_idx = 1;
                for EachNode(n, String8_Node, params->cmd_line.first->next) {
            ...
            if(params->inherit_env) {
                ...
                u64 env_idx = 0;
                for EachNode(n, String8_Node, params->cmd_line.first) {
...
internal Void_Proc * library_load_proc(Library lib, String8 name)
{
    ...
    Void_Proc *proc = (Void_Proc *)dlsym(so, name_cstr);
    scratch_end(scratch);
    return proc;
}
    ...
    for EachIndex(i, ips_count) {
        ...
        if(f) {
            ...
            if(fgets(func_name, sizeof(func_name), f) && fgets(file_name, sizeof(file_name), f)) {
                ...
                bool32 no_func = str8_match(func, str8_lit("??"), StringMatchFlag_RightSideSloppy);
                bool32 no_file = str8_match(file, str8_lit("??"), StringMatchFlag_RightSideSloppy);
