...
internal Stripe_Array
stripe_array_alloc(Arena *arena)
{
  Stripe_Array array = {0};
...
internal void
stripe_array_release(Stripe_Array *stripes)
{
...
internal Stripe *
stripe_from_slot_idx(Stripe_Array *stripes, u64 slot_idx)
{
...
typedef struct Barrier_TCTX Barrier_TCTX;
struct Barrier_TCTX
{
  Arena *arena;
  Barrier_Node *free_barrier_node;
};

thread_static Barrier_TCTX *barrier_tctx = 0;
...
slow_barrier_alloc(u64 count)
{
  if(barrier_tctx == 0)
  {
    ...
    barrier_tctx = push_array(arena, Barrier_TCTX, 1);
    barrier_tctx->arena = arena;
  }
  Barrier_Node *n = barrier_tctx->free_barrier_node;
  if(n != 0)
  {
  ...
  {
    n = push_array_no_zero(barrier_tctx->arena, Barrier_Node, 1);
  }
  MemoryZeroStruct(n);
  n->count = count;
...
slow_barrier_release(Barrier barrier)
{
  ...
  Barrier_Node *n = (Barrier_Node *)barrier.u64[0];
