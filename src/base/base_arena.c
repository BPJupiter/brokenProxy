...
internal void arena_release(Arena *arena)
{
#if PROFILE_TELEMETRY
    {
        ...
        tmPlot(0, TM_PLOT_UNITS_MEMORY, TM_PLOT_DRAW_LINE, 0, "%s\n%p", base_arena->name, base_arena);
    }
#endif
...
internal void *arena_push(Arena *arena, u64 size, u64 align, bool32 zero)
{
    ...
    // TODO: Newly allocated arenas already have zeroed committed pages,
    //       so this unnecessarily zeroes them again.
    // compute the size we need to zero out
    u64 size_to_zero = 0;
    if (zero) {
    ...
    if (current->reserve < pos_pst && !(arena->flags & ArenaFlag_NoChain)) {
...
#if ARENA_FREE_LIST
        {
            ...
            for (new_block = arena->free_last, prev_block = 0; new_block != 0; prev_block = new_block, new_block = new_block->prev) {
                if (new_block->reserve >= AlignPow2(new_block->pos, align) + size) {
                    if (prev_block) {
        ...
        if (new_block == 0) {
            ...
            u64 cmt_size = current->commit_size;
    ...
    if (current->commit < pos_pst) {
        u64 cmt_pst_aligned = pos_pst + current->commit_size - 1;
        cmt_pst_aligned -= cmt_pst_aligned % current->commit_size;
        u64 cmt_pst_clamped = ClampTop(cmt_pst_aligned, current->reserve);
        u64 cmt_size = cmt_pst_clamped - current->commit;
        u8 *cmt_ptr = (u8 *)current + current->commit;
        ...
        current->commit = cmt_pst_clamped;
    }
...
internal void arena_pop_to(Arena *arena, u64 pos)
{
    ...
    for (Arena *prev = 0; current->base_pos >= big_pos; current = prev) {
        ...
        AsanPoisonMemoryRegion((u8 *)current + ARENA_HEADER_SIZE, current->reserve - ARENA_HEADER_SIZE);
    }
#else
    for (Arena *prev = 0; current->base_pos >= big_pos; current = prev) {
        prev = current->prev;
        AsanUnpoisonMemoryRegion(current, current->commit);
        release_memory(current, current->reserve);
    }
#endif
...
#if PROFILE_TELEMETRY
    if ((pos - (new_pos + current->base_pos)) > Kilobytes(1)) {
        Arena *base_arena = arena;
        while (base_arena->prev) base_arena = base_arena->prev;
        tmPlot(0, TM_PLOT_UNITS_MEMORY, TM_PLOT_DRAW_LINE, (double)(current->base_pos + current->pos) / 1024.0 / 1024.0, "%s/%p", base_arena->name, base_arena);
    }
