...
void europa_current_time_get(uint32 *seconds, uint32 *fractions)
{
...
double europa_delta_time_compute(uint new_seconds, uint new_fractions, uint old_seconds, uint old_fractions)
{
...
#define EUROPA_WINDOWS_EPOCH_ADDITION 12591160000
#define EUROPA_WINDOWS_FRACTIONS 10000000

void europa_time_epoch_current(int64 *seconds, uint64 *fractions)
{
    VOID(WINAPI * e_GetSystemTimePreciseAsFileTime)(_Out_ LPFILETIME lpSystemTimeAsFileTime) = NULL;
...
#endif
        if (dll != NULL)
            e_GetSystemTimePreciseAsFileTime = (void(__stdcall *)(LPFILETIME lpSystemTimeAsFileTime))GetProcAddress((HMODULE)dll, "GetSystemTimePreciseAsFileTime");
        loaded = TRUE;
    }
    if (e_GetSystemTimePreciseAsFileTime != NULL)
        e_GetSystemTimePreciseAsFileTime(&filetime.st);
    else
        GetSystemTimeAsFileTime(&filetime.st);

    *seconds = (filetime.ui / EUROPA_WINDOWS_FRACTIONS) - EUROPA_WINDOWS_EPOCH_ADDITION;
    f64 = (filetime.ui % EUROPA_WINDOWS_FRACTIONS);
    f64 *= 0x0000000100000000;
    *fractions = (f64 / EUROPA_WINDOWS_FRACTIONS) * 0x0000000100000000;
}

#endif

/*
   void europa_time_epoc_test()
   {
    VOID(WINAPI *e_GetSystemTimePreciseAsFileTime)(_Out_ LPFILETIME lpSystemTimeAsFileTime) = NULL;
    static boolean loaded = FALSE;
 ...
 #endif
        if(dll != NULL)
            e_GetSystemTimePreciseAsFileTime = (void(__stdcall *)(LPFILETIME lpSystemTimeAsFileTime))GetProcAddress(dll, "GetSystemTimePreciseAsFileTime");
        loaded = TRUE;
    }
    if(e_GetSystemTimePreciseAsFileTime != NULL)
        e_GetSystemTimePreciseAsFileTime(&FileTime.st);
