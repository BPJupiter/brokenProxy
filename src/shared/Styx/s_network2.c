...
static void styxSocketDestroy(VSocket socket)
{
#if defined _WIN32
	closesocket(socket);
#else
...
boolean styxSocketInitWin32(void)
{
...
static VSocket styxSocketCreate(boolean stream, uint16 port, boolean ipv6)
{
	VSocket s;
...
boolean styxNetworkAddressLookup(StyxNetworkAddress *address, const char *dns_name, uint16 default_port, boolean *ipv6)
{
	char host[256];
	char port_str[16];
...
boolean styxNetworkAddressCompare(StyxNetworkAddress *a, StyxNetworkAddress *b)
{
...
static void styxSendPortMapping(boolean udp, uint16 port, boolean ipv6)
{
	/* IMPLEMENT ME */
}

SHandle *styxNetworkStreamAddressCreate(const char *host_name, uint16 port, boolean ipv6)
{
	SHandle *handle;
	VSocket socket;

	if (host_name == NULL) {
		socket = styxSocketCreate(TRUE, port, ipv6);
		if (socket == INVALID_VSOCKET) {
			return NULL;
		}
		styxSendPortMapping(FALSE, port, ipv6);
	}
	else {
		socket = styxSocketCreate(TRUE, 0, ipv6);
		if (socket == INVALID_VSOCKET) {
			return NULL;
		}
	}
	...
	if (host_name != NULL) {
		...
		if (styxNetworkAddressLookup(&ip, host_name, port, &ipv6)) {
			if (ip.is_ipv6) {
				...
				address_in.sin_addr.s_addr = ntohl(ip.ip.v4);	
	...
	if (host_name == NULL) {
		styxHandleClear(handle, S_HT_STREAMING_SERVER);
	}
	else {
		styxHandleClear(handle, S_HT_STREAMING_CONNECTION);
	}
	handle->port = port;
...
static void styxNetworkStreamAddressDestroy(SHandle *handle)
{
...
SHandle *styxNetworkStreamWaitForConnection(SHandle *listener, StyxNetworkAddress *from)
{
	SHandle *handle;
	VSocket socket;
	...
	styxNetworkWait(&listener, &read, NULL, 1, 1);
	if (read) {
...
boolean styxNetworkStreamConnected(SHandle *handle)
{
	return handle->connected;
}

SHandle *styxNetworkDatagramCreate(uint16 port, boolean ipv6)
{
	SHandle *handle;
	VSocket socket;
	socket = styxSocketCreate(FALSE, port, ipv6);
	if (socket == INVALID_VSOCKET) {
		return NULL;
	}
	handle = malloc(sizeof *handle);
...
int styxNetworkDatagramSend(SHandle *handle, StyxNetworkAddress *to)
{
	int out;
...
size_t styxNetworkStreamSend(SHandle *handle, uint8 *buffer, size_t length)
{
#if defined _WIN32
	int size;
#else
...
int styxNetworkStreamSendForce(SHandle *handle)
{
	size_t size;
...
int styxNetworkWait(SHandle **handles, boolean *read, boolean *write, uint handle_count, uint microseconds)
{
	struct timeval tv;
...
size_t styxNetworkStreamReceive(SHandle *handle, uint8 *buffer, size_t length)
{
#if defined _WIN32
	int get;
#else
...
int styxNetworkReceive(SHandle *handle, StyxNetworkAddress *from)
{
	struct sockaddr_storage address;
#if defined _WIN32
