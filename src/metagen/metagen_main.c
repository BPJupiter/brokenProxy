...
internal void entry_point(Cmd_Line *cmdline)
{
    // Set up state
    MG_Msg_List msgs = {0};
    mg_arena = arena_alloc(.reserve_size = Gigabytes(64), .commit_size = Megabytes(64));
    mg_state = push_array(mg_arena, MG_State, 1);
    mg_state->slots_count = 256;
    mg_state->slots = push_array(mg_arena, MG_Layer_Slot, mg_state->slots_count);
    ...
    // search code directories for all files to consider
    String8_List file_paths = {0};
    ...
    // parse all metadesk files
    MG_File_Parse_List parses = {0};
    DeferLoop(printf("parsing metadesk..."), printf(" %i metadesk files parsed\n", (int)parses.count)) {
        for (String8_Node *n = file_paths.first; n != 0; n = n->next) {
            ...
            if (str8_match(file_ext, str8_lit("mdesk"), 0)) {
                ...
                MD_Tokenise_Result tokenise = md_tokenise_from_text(mg_arena, data);
                MD_Parse_Result parse = md_parse_from_text_tokens(mg_arena, file_path, data, tokenise.tokens);
                for (MD_Msg *m = parse.msgs.first; m != 0; m = m->next) {
                    Txt_Pt pt = mg_txt_pt_from_string_off(data, m->node->src_offset);
                ...
                MG_File_Parse_Node *parse_n = push_array(mg_arena, MG_File_Parse_Node, 1);
        ...
        for (MG_File_Parse_Node *n = parses.first; n != 0; n = n->next) {
            ...
            for MD_EachNode(node, file->first) {
                ...
                if (!md_node_is_nil(table_tag)) {
                    MG_Node_Grid *table = push_array(mg_arena, MG_Node_Grid, 1);
                    MG_Column_Desc_Array *col_descs = push_array(mg_arena, MG_Column_Desc_Array, 1);
  ...
  for(MG_File_Parse_Node *n = parses.first; n != 0; n = n->next)
  {
    ...
    for MD_EachNode(node, file->first)
    {
      ...
      if(!md_node_is_nil(tag))
      {
        String8 layer_key = mg_layer_key_from_path(file->string);
        MG_Layer *layer = mg_layer_from_key(layer_key);
        bool32 prefer_c_file = md_node_has_tag(node, str8_lit("c_file"), 0);
        String8_List *out = prefer_c_file ? &layer->c_catchall : &layer->h_catchall;
      ...
      if(md_node_has_tag(node, str8_lit("markdown"), 0))
      {
        ...
        for(MD_Node *piece = node->first; !md_node_is_nil(piece); piece = piece->next)
        {
          ...
          if(md_node_has_tag(piece, str8_lit("p"), 0))
          {
            ...
            String8_List paragraph_lines = mg_wrapped_lines_from_string(mg_arena, paragraph_text, 80, 80, 0);
            for(String8_Node *n = paragraph_lines.first; n != 0; n = n->next)
            {
          ...
          {
            String8_List gen_strings = mg_string_list_from_table_gen(mg_arena, table_grid_map, table_col_map, str8_lit(""), piece);
            for(String8_Node *n = gen_strings.first; n != 0; n = n->next)
            {
              str8_list_pushf(mg_arena, &md_strs, " - ");
              String8 item_text = n->string;
              String8_List item_lines = mg_wrapped_lines_from_string(mg_arena, item_text, 80-3, 80, 3);
              for(String8_Node *line_n = item_lines.first; line_n != 0; line_n = line_n->next)
              {
  ...
  DeferLoop(printf("generating layer code..."), printf("\n"))
  {
    ...
    {
      MG_Layer_Slot *slot = &mg_state->slots[slot_idx];
      for(MG_Layer_Node *n = slot->first; n != 0; n = n->next)
      {
        MG_Layer *layer = &n->v;
        ...
        {
          String8_List layer_key_parts = str8_split_path(mg_arena, layer->key);
          String_Join join = {0};
          join.sep = str8_lit("_");
          ...
          }
          {
            ...
            else for(String8_Node *n = layer->h_header.first; n != 0; n = n->next)
            {
              fwrite(n->string.str, n->string.size, 1, h);
            }
            for(String8_Node *n = layer->enums.first; n != 0; n = n->next)
            {
              fwrite(n->string.str, n->string.size, 1, h);
            }
            for(String8_Node *n = layer->structs.first; n != 0; n = n->next)
            {
              fwrite(n->string.str, n->string.size, 1, h);
            }
            for(String8_Node *n = layer->h_catchall.first; n != 0; n = n->next)
            {
              fwrite(n->string.str, n->string.size, 1, h);
            }
            for(String8_Node *n = layer->h_functions.first; n != 0; n = n->next)
            {
              fwrite(n->string.str, n->string.size, 1, h);
            }
            if(layer->h_tables.first != 0)
            {
              ...
              for(String8_Node *n = layer->h_tables.first; n != 0; n = n->next)
              {
                fwrite(n->string.str, n->string.size, 1, h);
              }
              fprintf(h, "\n");
            ...
            else for(String8_Node *n = layer->h_footer.first; n != 0; n = n->next)
            {
              fwrite(n->string.str, n->string.size, 1, h);
            }
            fclose(h);
          }
          {
            ...
            else for(String8_Node *n = layer->c_header.first; n != 0; n = n->next)
            {
              fwrite(n->string.str, n->string.size, 1, c);
            }
            for(String8_Node *n = layer->c_catchall.first; n != 0; n = n->next)
            {
              fwrite(n->string.str, n->string.size, 1, c);
            }
            if(layer->c_tables.first != 0)
            {
              ...
              for(String8_Node *n = layer->c_tables.first; n != 0; n = n->next)
              {
                fwrite(n->string.str, n->string.size, 1, c);
              }
              if(!layer->is_library)
            ...
            for(String8_Node *n = layer->c_functions.first; n != 0; n = n->next)
            {
              fwrite(n->string.str, n->string.size, 1, c);
            }
            if(layer->c_footer.first != 0)
            {
              for(String8_Node *n = layer->c_footer.first; n != 0; n = n->next)
              {
                fwrite(n->string.str, n->string.size, 1, c);
              }
            }
  ...
  //
  for(MG_Msg_Node *n = msgs.first; n != 0; n = n->next)
  {
    MG_Msg *msg = &n->v;
